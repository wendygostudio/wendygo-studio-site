...
function proofSection(id,locale){
  ...
  return `<section class="section product-proof" data-product-proof data-version="${escapeHtml(p.version)}"><div class="section-inner"><h2 class="section-title">${es?'Prueba, privacidad y límites':'Proof, privacy and limits'}</h2><p class="hero-sub" style="margin:0">${es?'Información verificable para decidir antes de instalar.':'Verifiable details to help you decide before installing.'}</p><div class="proof-grid"><div><img class="proof-shot" src="${screenshot}" alt="${escapeHtml(p.name)} ${es?'vista del producto':'product preview'}" width="520" height="760" loading="lazy"><div class="proof-meta"><span>${es?'Versión':'Version'} ${escapeHtml(p.version)}</span><span>${es?'Chrome y Edge':'Chrome & Edge'}</span><span>${escapeHtml(p.locales.length)} ${es?'idiomas':'languages'}</span></div><a class="promo-link" href="${topic}">${es?'Ver guías relacionadas →':'See related guides →'}</a></div><div><article class="proof-card"><h3>${es?'Cómo trata tus datos':'How your data is handled'}</h3><p>${privacy}</p></article><article class="proof-card"><h3>${es?'Permisos declarados':'Declared permissions'}</h3><ul>${permissions}</ul></article><article class="proof-card"><h3>${es?'Limitaciones conocidas':'Known limitations'}</h3><ul>${limitations}</ul></article><article class="proof-card"><h3>${es?'Fuente oficial':'Official listing'}</h3><p>${es?'Consulta la ficha de Chrome Web Store para la versión publicada, permisos y notas de actualización.':'Check the Chrome Web Store listing for the published version, permissions and update notes.'}</p><a class="promo-link" href="https://chromewebstore.google.com/detail/${escapeHtml(p.storeId)}" target="_blank" rel="noopener">Chrome Web Store →</a></article></div></div></div></section>`;
}
