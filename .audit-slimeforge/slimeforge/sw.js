...
import { loadState, saveState, applyElapsed, attentionNeeded, isAsleep, dayKey, addGrowth, huertoOp, FOCUS_DEMO_MS } from './common/logic.js';
    ...
    switch (msg.type) {
      ...
      case 'sf-focus': {
        ...
        if (msg.op === 'start' && !s.focus) {
          ...
          const dur = s.demoMode ? (isBreak ? 15000 : FOCUS_DEMO_MS) : (isBreak ? 5 * 60000 : (s.focusMin || 25) * 60000);
          s.focus = { startedAt: Date.now(), endsAt: Date.now() + dur, duration: dur, kind: isBreak ? 'break' : 'work' };
        ...
        if (msg.op === 'extend' && s.focus && s.focus.kind !== 'break') {
          s.focus.endsAt += 5 * 60000; s.focus.duration += 5 * 60000;
          await chrome.alarms.create('focusEnd', { when: s.focus.endsAt + 500 });
      ...
      case 'sf-duel': {
        ...
        if (msg.won) { s.brasas += 8; s.animo = Math.min(100, s.animo + 5); addGrowth(s, 5); s.duelsW = (s.duelsW || 0) + 1; }
        await saveState(s);
        break;
      }
      case 'sf-laser':
