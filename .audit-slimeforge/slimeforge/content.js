...
(function () {
  ...
  const L_STRINGS = {
    es: { mine: '¡Mías! +3 🔥', mine_sock: '¡Mías! +3 🔥 …¿y un 🧦?', box_sock: '🧦 ¡Había un calcetín en la caja!',
          ...
          duel_hi: '¡Eh, tú! Duelo de ingenio. Si te atreves. ⚔️', duel_pick: 'Elige tu réplica:', duel_hit: '¡Zas! Directa.', duel_miss: 'Uy… esa no era.', duel_learn: 'La buena era: «$R»', duel_win: '¡Victoria! +8 🔥', duel_lose: 'Me ha ganado… esta vez. Réplica aprendida.',
    ...
    en: { mine: 'Mine! +3 🔥', mine_sock: 'Mine! +3 🔥 …and a 🧦?', box_sock: '🧦 There was a sock in the box!',
          ...
          duel_hi: 'Hey, you! Duel of wits. If you dare. ⚔️', duel_pick: 'Choose your comeback:', duel_hit: 'Zing! Direct hit.', duel_miss: 'Oof… not that one.', duel_learn: 'The right one was: “$R”', duel_win: 'Victory! +8 🔥', duel_lose: 'It beat me… this time. Comeback learned.',
    ...
    de: { mine: 'Meins! +3 🔥', mine_sock: 'Meins! +3 🔥 …und eine 🧦?', box_sock: '🧦 In der Kiste war eine Socke!',
          ...
          duel_hi: 'He, du! Duell der Schlagfertigkeit. Wenn du dich traust. ⚔️', duel_pick: 'Wähle deine Antwort:', duel_hit: 'Zack! Volltreffer.', duel_miss: 'Uff… die war es nicht.', duel_learn: 'Richtig wäre gewesen: „$R“', duel_win: 'Sieg! +8 🔥', duel_lose: 'Es hat mich besiegt… diesmal. Antwort gelernt.',
    ...
    fr: { mine: 'À moi ! +3 🔥', mine_sock: 'À moi ! +3 🔥 …et une 🧦 ?', box_sock: '🧦 Il y avait une chaussette dans la boîte !',
          ...
          duel_hi: 'Hé, toi ! Duel d\'esprit. Si tu oses. ⚔️', duel_pick: 'Choisis ta réplique :', duel_hit: 'Paf ! En plein dans le mille.', duel_miss: 'Aïe… ce n\'était pas celle-là.', duel_learn: 'La bonne était : « $R »', duel_win: 'Victoire ! +8 🔥', duel_lose: 'Il m\'a battu… cette fois. Réplique apprise.',
    ...
    it: { mine: 'Mie! +3 🔥', mine_sock: 'Mie! +3 🔥 …e un 🧦?', box_sock: '🧦 C\'era un calzino nella scatola!',
          ...
          duel_hi: 'Ehi, tu! Duello d\'ingegno. Se hai coraggio. ⚔️', duel_pick: 'Scegli la tua replica:', duel_hit: 'Zac! Colpo diretto.', duel_miss: 'Ahi… non era quella.', duel_learn: 'Quella giusta era: «$R»', duel_win: 'Vittoria! +8 🔥', duel_lose: 'Mi ha battuto… stavolta. Replica imparata.',
    ...
    pt: { mine: 'Minhas! +3 🔥', mine_sock: 'Minhas! +3 🔥 …e uma 🧦?', box_sock: '🧦 Havia uma meia na caixa!',
          ...
          duel_hi: 'Eh, tu! Duelo de engenho. Se te atreves. ⚔️', duel_pick: 'Escolhe a tua réplica:', duel_hit: 'Zás! Direta.', duel_miss: 'Ui… não era essa.', duel_learn: 'A certa era: «$R»', duel_win: 'Vitória! +8 🔥', duel_lose: 'Ganhou-me… desta vez. Réplica aprendida.',
  ...
  let S = null, host = null, root = null, wrap = null, svgEl = null, bubbleEl = null;
  ...
  function renderFocusHud() {
    ...
    const focus = S.focus;
    const sig = (focus ? 'f' + focus.kind + (focus.startedAt || 0) + '-' + (focus.endsAt || 0) : 'idle' + (S.focusMin || 25)) + '|' + (conf.op || 85) + '|' + (conf.hx == null ? 'd' : conf.hx + ',' + conf.hy);
  ...
  function loopTick(now) {
    if (S && S.phase === 'pet' && wrap) {
      ...
      if (!dragging) {
        ...
        tickVisit(now);
        if (!asleep && !ev && !visit && !S.focus && now > nextEventAt) { nextEventAt = now + eventDelay(); startEvent(); }
        tickEvent(now);
        if (!packEv && !visit && !ev && !asleep && !S.focus && comps.length >= 1 && now > nextPackAt) {
        ...
        if (!asleep && !ev && !visit && !packEv && !S.focus && now > nextSoloAt) soloThought(now);
        if (!asleep && !ev && !visit && !packEv && !dragging && !S.focus && mode === 'idle' && now > nextMicroAt) microGesture(now);
