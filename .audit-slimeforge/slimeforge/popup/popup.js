...
import { proActive, proTier, FORCE_PRO, PAY_LINKS, activateLicense, revalidateLicense, deactivateLicense, trialStatus, startTrial } from '../common/license.js';
import {
  loadState, saveState, freshState, applyElapsed, addGrowth, markDiscovered,
  huertoOp, cosUnlocked, cosProgress, FOCUS_REAL_MS, FOCUS_DEMO_MS, dayKey
...
$('selLang').addEventListener('change', async e => {
  ...
  translateDOM();
  renderPro(); renderPets(); renderBestiario(); renderDaily(); renderCraft(); renderNanoUI(); updateHUD();
  persist();
});
...
async function loadI18N(lang) {
  ...
  } catch (e) { if (lang !== 'es') await loadI18N('es'); }
}
function t(key, ...subs) {
  let m = I18N[key];
...
$('aiDiag').addEventListener('click', async () => {
  ...
  $('aiDiag').textContent = '🔬';
  openModal('🧠 Nano · diagnóstico');
  mbody.innerHTML = !r
    ? '<div class="mnote">Sin respuesta del service worker.</div>'
    : '<div class="mnote" style="line-height:1.7">' +
      'LanguageModel en el SW: <b>' + (r.lm ? 'sí' : 'NO — el SW no ve Nano') + '</b><br>' +
      'Disponibilidad: <b>' + (r.avail || '—') + '</b><br>' +
      (r.ms != null ? '1ª línea (sesión fría): <b>' + r.ms + ' ms</b>' + (r.ms > 4500 ? ' ⚠️ supera el presupuesto' : '') + '<br>' : '') +
      (r.line ? 'Respuesta: <i>«' + r.line + '»</i><br>' : '') +
      (r.err ? 'Error: <code>' + r.err + '</code>' : '') +
      '</div>';
});
...
function renderPro() {
  ...
  const tm = $('trialMini');
  tm.style.display = (tb.style.display !== 'none' && G.phase === 'pet') ? '' : 'none';
  tm.textContent = tb.textContent;
  renderProPerks();
  if (FORCE_PRO) {
    st.innerHTML = '🧪 <b style="color:var(--amber)">Build de pruebas: Pro desbloqueado.</b> Todo abierto para testing.';
...
const PRO_PERKS = [
  { k: 'multi',   on: s => s.stable && s.stable.some(p => p) },
  { k: 'pack',    on: s => !!s.packMode },
  { k: 'reserve', on: s => (s.reserve || []).length > 10 },
  { k: 'cos',     on: s => s.dna && s.dna.cos && Object.keys(COS).some(id => COS[id].pro && Object.values(s.dna.cos).includes(id)) },
  { k: 'focus',   on: () => true },
  { k: 'acc',     on: s => s.dna && (s.dna.accessory === 'gorro' || s.dna.accessory === 'monoculo') },
  { k: 'forge',   on: s => s.forjas && s.forjas.month === monthKey() && s.forjas.used > 0 }
];
function renderProPerks() {
  ...
  box.innerHTML = head + '<div class="perklist">' + rows + '</div>' + foot;
}
...
const PET_FIELDS = ['phase', 'cracks', 'dna', 'stageIdx', 'growth', 'hambre', 'higiene', 'animo', 'vinculo', 'educacion', 'poops', 'poopBornAt', 'sick', 'sickMeds', 'sickMimos', 'neglectMs', 'visits', 'wokeUntil', 'digestBoost', 'pendingPoopShow', 'sizePct'];
...
function freshEggPet() {
  return { phase: 'egg', cracks: 0, dna: null, stageIdx: 0, growth: 0, hambre: 80, higiene: 90, animo: 70, vinculo: 5, educacion: 30, poops: 0, poopBornAt: 0, sick: false, sickMeds: false, sickMimos: 0, neglectMs: 0, visits: [], wokeUntil: 0, digestBoost: false, pendingPoopShow: false, sizePct: 100 };
}
function renderPets() {
  const box = $('petsbox');
  ...
  const activeMini = G.phase === 'pet'
    ...
    : '🥚';
  rows.push('<div class="petslot"><div class="mini">' + activeMini + '</div><div class="pnm"><b>' + (G.dna ? G.dna.name : t('egg_word')) + '</b><small>' + t('pets_active') + '</small></div><input type="range" class="psz" min="10" max="100" data-psize="a" value="' + (G.sizePct == null ? 100 : G.sizePct) + '" title="' + t('lbl_size') + ' ' + (G.sizePct == null ? 100 : G.sizePct) + '%" style="width:56px"><span style="color:var(--amber);font-size:0.74rem;font-weight:700">● ' + t('pets_withyou') + '</span></div>');
  for (let i = 0; i < 2; i++) {
    const p = G.stable[i];
...
function showBestLore(sp) {
  ...
  lore.style.display = '';
  lore.innerHTML = '<b>' + spName(sp) + (RARITY[sp] ? ' · <span style="color:var(--amber)">' + RARITY[sp].toUpperCase() + '</span>' : '') + '</b><br>' + (SP_LORE[sp] || '');
}
...
function renderReserve() {
  ...
  $('resList').innerHTML = res.map((p, j) => {
    const mini = (p.phase === 'pet' && p.dna)
      ...
      : '🥚';
    return '<div class="petslot"><div class="mini" style="width:44px;height:44px">' + mini + '</div><div class="pnm"><b>' + (p.dna ? p.dna.name : t('egg_word')) + '</b><small>' + (p.dna ? spName(p.dna.species) + ' · ' + stName(STAGES[p.stageIdx].id) : t('res_unhatched')) + '</small></div>' +
...
function dailyGift() {
  ...
  let msg;
  if (roll < 0.55) { const n = 2 + Math.floor(Math.random() * 3); G.brasas += n; msg = '+' + n + ' 🔥'; }
...
function sig() {
  return [G.phase, G.cracks, G.stageIdx, dirtLevel(), G.poops - hidePoops,
    G.focus ? (G.focus.kind || 'work') + (G.focus.startedAt || 0) : 0, !!G.sick, isAsleep(G), G.animo < 30, G.animo > 85,
    G.dna && (G.dna.seed + '-' + G.dna.accessory + '-' + (G.dna.cos ? Object.values(G.dna.cos).join('.') : ''))].join('|');
}
...
function updateFocusUI() {
  ...
  $('selMin').value = String(G.focusMin || 25);
  ...
  if (G.focus) {
    ...
    $('ftimer').textContent = fmt(G.demoMode ? FOCUS_DEMO_MS : (G.focusMin || 25) * 60000);
...
function goTab(name) {
  ...
  document.querySelectorAll('.tab').forEach(b => b.classList.toggle('on', b.getAttribute('data-goto') === name));
...
function syncPhaseUI() {
  ...
  ['bars', 'growth', 'actions', 'minis', 'huertoBox', 'screenbox', 'visitbox', 'craftbox', 'cosBox', 'logrosBox', 'dailybox', 'savebox'].forEach(id => $(id).style.display = pet ? '' : 'none');
...
$('fileImport').addEventListener('change', async e => {
  ...
  try {
    ...
    if (!st || typeof st !== 'object' || !st.phase) throw new Error('bad');
    if (!confirm(t('imp_confirm'))) return;
    const cands = [st.trialStart, G && G.trialStart, Date.now()].filter(v => typeof v === 'number' && v > 0);
    st.trialStart = Math.min.apply(null, cands);
    // el high-water mark tampoco rejuvenece: conservar el MAYOR visto entre
    // archivo e instalación (si no, importar un guardado viejo + reloj atrás
    // reabriría el trial ya consumido)
    st.trialSeen = Math.max(st.trialSeen || 0, (G && G.trialSeen) || 0);
    G = st;
    await persist();
    location.reload();
...
async function persist() { G.writerId = WRITER_ID; await saveState(G); }
chrome.storage.onChanged.addListener((changes, area) => {
  ...
  G = nv;
  lastSig = '';
  syncPhaseUI(); renderPro(); renderPets(); draw(true); updateHUD(); drainToasts();
});
...
function offerRoam() {
  ...
  if (G.roamAsked) return;
  G.roamAsked = true; persist();
  const ov = document.createElement('div');
  ov.className = 'ob-ov';
  const card = document.createElement('div');
  card.className = 'ob-card';
  card.innerHTML = '<div class="ob-egg">\u{1F43E}</div><h2>' + t('hatch_roam_t', { NAME: G.dna.name }) + '</h2><p>' + t('hatch_roam_d') + '</p>' +
  ...
  card.querySelector('#roamLater').addEventListener('click', () => ov.remove());
...
function celebrateHatch() {
  ...
  if (RARITY[G.dna.species]) {
    setTimeout(() => { toast('🌟 ' + t('rarity', { R: RARITY[G.dna.species].toUpperCase() })); sfx.sparkle(); }, 1200);
  }
  draw(true, true); updateHUD(); renderBestiario(); persist();
  setTimeout(offerRoam, 2600);   // tras el toast de nacimiento
}
...
$('btnFeed').addEventListener('click', () => {
  ...
  react('happy', 2200);
  if (G.peces > 0) {
    G.peces--; G.hambre = Math.min(100, G.hambre + 32); G.animo = Math.min(100, G.animo + 4);
    evolveFx(addGrowth(G, 8));
    sfx.eat(); fx('munch', 850);
    particle('🐟'); toast(t('feed_fish'));
  } else {
    if (G.brasas < 2) return;
    G.brasas -= 2; G.hambre = Math.min(100, G.hambre + 30); G.animo = Math.min(100, G.animo + 3);
    evolveFx(addGrowth(G, 8));
    sfx.eat(); fx('munch', 850);
...
$('btnShower').addEventListener('click', () => {
  ...
  setTimeout(() => {
    ...
    if (wasDirty) {
      ...
      evolveFx(addGrowth(G, 8));   // el growth se gana CUIDANDO, no spameando: solo si estaba sucia (<50)
    }
    fx('wiggle', 650);
    toast(t('shower_done'));
    draw(true); updateHUD(); persist();
...
const FOOD_FX = { baya: { hambre: 18, animo: 4 }, zanahoria: { hambre: 40, animo: 0 } };
function fmtH(ms) {
...
$('btnFocus').addEventListener('click', async () => {
  const dur = G.demoMode ? FOCUS_DEMO_MS : (G.focusMin || 25) * 60000;
  G.focus = { startedAt: Date.now(), endsAt: Date.now() + dur, duration: dur, kind: 'work' };
...
$('btnBreak').addEventListener('click', async () => {
  if (G.focus) return;
  const dur = G.demoMode ? 15000 : 5 * 60000;
  G.focus = { startedAt: Date.now(), endsAt: Date.now() + dur, duration: dur, kind: 'break' };
  await chrome.alarms.create('focusEnd', { when: G.focus.endsAt + 500 });
  toast('☕ ' + t('focus_break'));
  draw(true); updateHUD(); persist();
});
...
function pescaIdle() {
  if (fsh.cast >= fsh.max) {
    ...
    toast('🎣 ' + t('mg_pesca_toast', { B: brasas, F: peces }));
    return;
  }
...
function ovilloEnd(score) {
  ...
  const brasas = Math.max(1, Math.min(8, Math.round(pts / 22)));
  ...
  mbody.innerHTML = '<div class="pond">🧶✨</div>' +
    '<div class="mnote">' + t('mg_ovillo_result', { P: pts, B: brasas, H: hilo ? ' +' + hilo + '🧵' : '', N: G.dna ? G.dna.name : '???' }) + '</div>' +
  ...
  toast('🧶 ' + t('mg_ovillo_toast', { B: brasas }));
}
...
function renderVisits() {
  ...
  box.innerHTML = v.length
    ? t('visit_mems') + ' ' + v.map(m => visitEmoji(m.score) + ' <b>' + m.name + '</b>' + (m.owner ? ' (' + m.owner + ')' : '') + (m.bff ? ' 💛' : '')).join(' · ')
    : t('visit_none');
}
...
$('btnVisit').addEventListener('click', () => {
  if (visiting) return;
  let p = null;
  try { p = JSON.parse(decodeURIComponent(escape(atob($('visitCode').value.trim())))); } catch (e) {}
...
/* ═══ LA FORJA (editor · Pro, beta abierta) ═══ */
...
(async function init() {
  ...
  document.body.classList.toggle('light', !!G.lightTheme);
  $('btnTheme').textContent = G.lightTheme ? '☀️' : '🌙';
  const uiLang = G.uiLang || navigator.language.slice(0,2);
  if (['es','en','de','fr','pt_PT','it'].includes(uiLang)) $('selLang').value = uiLang;
  startTrial(G);
  G.proOn = proActive(G);
  const uiL = G.uiLang || navigator.language.slice(0, 2);
  await loadI18N(['es','en','de','fr','pt_PT','it'].includes(uiL) ? uiL : 'es');
  await persist();
  translateDOM();
  renderPro();
  renderPets();
  renderBestiario();   // sin esto, la pestaña Colección abría vacía hasta forjar o cambiar de idioma
  revalidateLicense(G).then(changed => { if (changed) { G.proOn = proActive(G); persist(); renderPro(); renderPets(); updateHUD(); } });
  draw(true);
  updateHUD();
  drainToasts();
  ...
  goTab(G.uiTab || 'foco');
  if (G.pendingHatch) { G.pendingHatch = false; setTimeout(celebrateHatch, 600); }   // eclosionó con el popup cerrado
  ...
  setInterval(() => {
    ...
    if (G.phase === 'pet' && !isAsleep(G) && popupEvT >= 40 && Math.random() < 0.05 * chatMod) {
      popupEvT = 0;
      popupEvent();
    }
  }, 1000);
