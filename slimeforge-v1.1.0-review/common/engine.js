...
export const STAGES = [
  { id:'chispa',  label:'Chispa',  scale:0.20, next:50 },
  { id:'cria',    label:'Cría',    scale:0.40, next:200 },
  { id:'joven',   label:'Joven',   scale:0.62, next:500 },
...
export function buildCreature(dna, stageIdx, extras) {
  ...
  const s = Number.isFinite(extras.scaleOverride) ? extras.scaleOverride : scaleFor(stageIdx, extras.growth);
  ...
  const inner = `
    ...
    <g class="${ghost ? 'ghostfloat' : ''}" ${ghost ? 'opacity="0.92"' : ''}>
      <g class="squish species-${sp}" data-species="${sp}" data-action="pet" style="cursor:pointer">
